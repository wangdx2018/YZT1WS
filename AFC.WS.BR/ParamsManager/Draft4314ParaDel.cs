...
namespace AFC.WS.BR.ParamsManager
{
    class Draft4314ParaDel
    {
        ...
        public static int DelParaVersionInfo4314(string paraType, string version)
        {
            int res = 0;
        ...
        {
            int res = 0;
            string delSql = string.Format("delete from para_4314_autorun_time t where t.para_version='-1'", paraType, version);
            try
            {
                Util.DataBase.SqlCommand(out res, delSql);
                return res;

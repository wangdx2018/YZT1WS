    ...
    public class RunManager
    {
        ...
        public DataTable GetAllTikcetRunDateStatus(string stationId, string rundate)
        {
            try
            {
                string cmd = "select btmt.tick_mana_type_name as 票卡类型,"
                  ...
                  +" t.tick_mana_type where t.station_id like '" + stationId + "' and t.update_date = '" + rundate + "'";
                return DBCommon.Instance.GetDatatable(cmd);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetAllMoneyRunDateStatus(string stationId, string rundate)
        {
            try
            {
               ...
               + "t.currency_code where t.station_id like '" + stationId + "' and t.update_date = '" + rundate + "'";
                return DBCommon.Instance.GetDatatable(cmd);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns>如果处在运营开始返回true，否则返回false</returns>
        public bool CheckPartDevStart()
        {
            bool isStart = false;
            string cmd = string.Format("select t.* from dev_run_status_detail t where t.status_id='0A01' and t.status_value='00' and t.station_id='{0}'", SysConfig.GetSysConfig().LocalParamsConfig.StationCode);
            try
            {
                DataTable dt = DBCommon.Instance.GetDatatable(cmd);
                if (dt != null && dt.Rows.Count > 0)
        ...
        {
            //todo: get data from basi_run_param_info
            string cmd = "select sum(t.original_money-t.later_money) from cash_store_change_log t where t.change_type = '04' and t.station_id like '" + stationId + "' and t.update_date = '" + runDate + "'";
            string CashTotalStoreValue = string.Empty;
            try
            {
                DataTable dt = DBCommon.Instance.GetDatatable(cmd);
        ...
        public string GetTodayCoinTotalValue(string stationId, string runDate)
        {
            ...
            string cmd = "select sum(t.original_money - t.later_money) as total_money "
               ...
               + "and t.station_id like '" + stationId + "' and t.update_date = '" + runDate + "'";
        ...
        public string GetTomorrowIncomeAmount(string stationId, string runDate)
        {
            ...
            string cmd = "select sum(nvl(t.key_fld2,0)) from dev_double_oper_log_info t where t.busi_code = '1615'"
               + " and t.station_id like '" + stationId + "' and t.oper_date = '" + runDate + "'";
            string TomorrowIncomeAmount = string.Empty;
            try
            {
                DataTable dt = DBCommon.Instance.GetDatatable(cmd);
        ...
        {
            //todo: get data from basi_run_param_info
            string cmd = "select t.today_diff_amount from cash_date_settlement_info t where"
               + " t.station_id like '" + stationId + "' and t.run_date = '" + runDate + "'";
        ...
        {
            //todo: get data from basi_run_param_info
            string cmd = "select sum(nvl(t.in_store_num,0)) as all_total_num from tick_storage_info t where t.station_id like '" + stationId + "' and t.update_date = '" + rundate + "'";
        ...
        {
            //todo: get data from basi_run_param_info
            string cmd = "select sum(t.total_encash_coin_sum-t.total_change_coin_sum +t.total_encash_bill_sum-t.total_change_bill_sum) as income_sum from data_tvm_settlement_info t where t.station_id like '" + stationId + "' and t.settlement_date = '" + rundate + "'";
            string CashTotalStoreValue = string.Empty;
            try
            {
                DataTable dt = DBCommon.Instance.GetDatatable(cmd);

...
{
    public class BusiDataExportAction:IAction
    {
        BusiDataExport busiExport = AbstractDataExport.CreateInstance(OperateType.BUSI_DATA_FILE) as BusiDataExport;
        ...
        {
            //FTPInfo ftpInfo = busiExport.GetFtpInfo();
            ...
            if (condition == null)
                return null;
            int res = busiExport.ExportFiles(condition);
            switch (res)
            {
                case -1:
            ...
            res = busiExport.RewriteFile(memIndexPath);
            if (res == -1)
            {
            ...
            ///记录操作日志
            //res = busiExport.ExportSuccHandle();
            //if (res == -1)
            //{
            ...
            MessageDialog.Show("业务数据导出成功！等待后台处理！", "提示！", MessageBoxIcon.Information, MessageBoxButtons.Ok);

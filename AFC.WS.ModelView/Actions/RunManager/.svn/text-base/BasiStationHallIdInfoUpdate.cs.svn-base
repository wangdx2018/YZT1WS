...
{
    public class BasiStationHallIdInfoUpdate : IAction
    {
        #region IAction 成员
        ...
        public bool CheckValid(List<QueryCondition> actionParamsList)
        {
            ...
            {
                MessageDialog.Show("请选择要更新的车站站厅", "提示", MessageBoxIcon.Information, MessageBoxButtons.Ok);
                return false;
            }
        ...
        public ResultStatus DoAction(List<QueryCondition> actionParamsList)
        {
            ...
            //车站站厅更新
            res = DBCommon.Instance.UpdateTableAll(info, "basi_station_hall_id_info", new KeyValuePair<string, string>("station_id", BuinessRule.GetInstace().GetStationInfoByName(StationName).station_id.ToString()), new KeyValuePair<string, string>("station_hall_id", StationHallIdOld));
            if (res == 1)
            {
                Wrapper.ShowDialog("车站站厅更新成功。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.basi_station_hall_id_info_Update, "0", "车站站厅更新成功");
                Util.DataBase.Commit();
                return null;
            }
            else
            {
                Wrapper.ShowDialog("车站站厅更新失败。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.basi_station_hall_id_info_Update, "1", "车站站厅更新失败");

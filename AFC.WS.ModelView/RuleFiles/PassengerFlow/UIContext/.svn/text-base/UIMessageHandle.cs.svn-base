    ...
    internal class UIMessageHandle : IMessageHandler, IControllerMessageHandler
    {
        ...
        public void HandleAsynMessage(Message msg)
        {
            this.myWindow.Dispatcher.Invoke(new Fun(() =>
            {
                ...
                if (msg.MessageParam is ModeChange_1341)
                {
                    ModeChange_1341 body = msg.MessageParam as ModeChange_1341;
                    if (body != null)
                    {
                        BasiStationInfo bsi = BuinessRule.GetInstace().GetStationInfoByDevId(body.mode_station_id.ToString("X8"));
                         string stationMode = GetModeCodeInfo(body.mode_code);
                        if (bsi.station_id.Equals(SysConfig.GetSysConfig().LocalParamsConfig.StationCode))
                        {
                            stationMode = GetModeCodeInfo(body.mode_code);
                        ...
                        {
                            MessageDialog.Show(string.Format("{0} 发生 {1} 模式", bsi.station_cn_name, stationMode),
        ...
        private void HandleLogOutMessage()
        {
            ...
            if (res != 0)
            {
                AFC.WS.UI.CommonControls.MessageDialog.Show("LogOut Failed!", "Tip", MessageBoxIcon.Information, MessageBoxButtons.Ok);

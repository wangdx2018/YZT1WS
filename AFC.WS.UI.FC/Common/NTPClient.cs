...
namespace AFC.WS.UI.Common
{
    ...
    public class NTPClient
    {
            ...
            get
            {
                ...
                switch (Stratum)
                {
                    ...
                    case _Stratum.PrimaryReference:
                        val += NTPData[offReferenceID + 0].ToString();
                        val += NTPData[offReferenceID + 1].ToString();
                        val += NTPData[offReferenceID + 2].ToString();
                        val += NTPData[offReferenceID + 3].ToString();
                        break;
                    case _Stratum.SecondaryReference:
                        switch (VersionNumber)
                        {
                            case 3:	// Version 3, Reference ID is an IPv4 address
                                byte[] Address ={ NTPData[offReferenceID + 0] ,
        ...
        public int Connect()
        {
            try
            {
                IPAddress[] hostadd = Dns.GetHostAddresses(TimeServer);
                IPEndPoint EPhost = new IPEndPoint(hostadd[0], 123);

...
{
    public class ParamDataExport:AbstractDataExport
    {
        ...
        private string GetSQl(ConditionClass cc)
        {
            ...
            {
                strSQL = "select * from para_local_full_ver_info t where t.para_master_type='4300' and (t.edition_type='0' or t.edition_type='1') and t.para_file_name is not null";
            }

            //ACC参数
            if (cc.paraType == "02" && cc.verType == "01")
            {
                strSQL = "select * from para_local_full_ver_info t where t.edition_type='0' and (t.para_master_type='4001' or t.Para_Master_Type='4002') and t.para_file_name is not null";
            }
            if (cc.paraType == "02" && cc.verType == "02")
            {
                strSQL = "select * from para_local_full_ver_info t where t.edition_type='1' and (t.para_master_type='4001' or t.Para_Master_Type='4002') and t.para_file_name is not null";
            }
            if (cc.paraType == "02" && cc.verType == "03")
            {
                strSQL = "select * from para_local_full_ver_info t where (t.para_master_type='4001' or t.Para_Master_Type='4002') and (t.edition_type='0' or t.edition_type='1') and t.para_file_name is not null";
            }

            //ACC和LCC
            if (cc.paraType == "03" && cc.verType == "01")
            {
                strSQL = "select * from para_local_full_ver_info t where t.edition_type='0' and t.para_file_name is not null";
            }
            if (cc.paraType == "03" && cc.verType == "02")
            {
                strSQL = "select * from para_local_full_ver_info t where t.edition_type='1' and t.para_file_name is not null";
            }
            if (cc.paraType == "03" && cc.verType == "03")
            {
                strSQL = "select * from para_local_full_ver_info t where (t.edition_type='0' or t.edition_type='1') and t.para_file_name is not null";
            }
        ...
        {
            IndexFileData fileData = IndexFileData.CreateIndexFileData(OperateType.PARA_DATA_FILE);
        ...
        public override int RewriteFile(string indexFileName)
        {
            IndexFileData fileData = SetParaData();
            int res = 0;
            ParaIndexFileHandle paraHandle = new ParaIndexFileHandle();
            res = paraHandle.CreateIndexFile(indexFileName, fileData);
            if (res != 0)
                return -1;
            return res;
        }
        ...
        {
            //todo:记录操作日志
            int res = BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.para_data_import, "0", "导出参数数据成功！");
            if (res != 0)
                return -1;
            return res;
        }

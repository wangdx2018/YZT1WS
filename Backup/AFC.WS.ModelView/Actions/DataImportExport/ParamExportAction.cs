...
{
    public class ParamExportAction:IAction
    {
        ParamDataExport paraExport = AbstractDataExport.CreateInstance(OperateType.PARA_DATA_FILE) as ParamDataExport;
        ...
        {
            //FTPInfo ftpInfo = paraExport.GetFtpInfo();
            string paraIndexPath = usbOper.getFirstUSB();
            ConditionClass condition = actionParamsList.Single(temp => temp.bindingData.Equals("Condition")).value as ConditionClass;
            if (condition == null)
                return null;

            int res = paraExport.ExportFiles(condition);
            switch (res)
            {
                case -1:
            ...
            res = paraExport.RewriteFile(paraIndexPath);
            if (res == -1)
            {
            ...
            ///记录操作日志
            //res = paraExport.ExportSuccHandle();
            //if (res == -1)
            //{
            ...
            MessageDialog.Show("参数导入成功！等待后台处理！", "提示！", MessageBoxIcon.Information, MessageBoxButtons.Ok);

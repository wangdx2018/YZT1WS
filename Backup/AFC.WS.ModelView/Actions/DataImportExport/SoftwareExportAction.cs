...
{
    public class SoftwareExportAction:IAction
    {
        SoftwareDataExport softExport = AbstractDataExport.CreateInstance(OperateType.SOFT_DATA_FILE) as SoftwareDataExport;
        ...
        {
            //FTPInfo ftpInfo = softExport.GetFtpInfo();
            ...
            int res = softExport.ExportFiles(condition);
            switch (res)
            {
                case -1:
            ...
            res = softExport.RewriteFile(paraIndexPath);
            if (res == -1)
            {
            ...
            ///记录操作日志
            //res = softExport.ExportSuccHandle();
            //if (res == -1)
            //{

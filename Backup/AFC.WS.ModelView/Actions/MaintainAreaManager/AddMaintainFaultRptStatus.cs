...
namespace AFC.WS.ModelView.Actions.MaintainAreaManager
{
    ...
    public class AddMaintainFaultRptStatus:  IAction
    {
        #region IAction 成员
        ...
        public ResultStatus DoAction(List<QueryCondition> actionParamsList)
        {
            ...
            info.fault_doc_id = device_id + DateTime.Now.ToString("yyyyMMddHHmmss");
            ...
            info1.fault_doc_id = device_id + DateTime.Now.ToString("yyyyMMddHHmmss");
            info1.status = "01";
            info1.remark = remark;
            ...
            if (res == 1 && res1==1)
            {
                Wrapper.ShowDialog("故障单信息添加成功。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.Maintain_Fault_Rpt_Status_Add, "0", "故障单信息添加成功");
                Util.DataBase.Commit();
                return null;
            }
            else if (res != 1 && res1 == 1)
            {
                Wrapper.ShowDialog("故障单信息添加失败。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.Maintain_Fault_Rpt_Status_Add, "1", "故障单信息添加失败");
            ...
            {
                Wrapper.ShowDialog("故障单信息添加失败。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.Maintain_Fault_Rpt_Status_Add, "1", "故障单跟踪信息添加失败");
                Util.DataBase.Rollback();
                return null;
            }
            else if (res != 1 && res1 != 1)
            {
                Wrapper.ShowDialog("故障单信息添加失败。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.Maintain_Fault_Rpt_Status_Add, "1", "故障单信息、故障单跟踪信息添加失败");
                Util.DataBase.Rollback();
                return null;
            }
            return null;

...
{
    public class AddPara4042DeviceInfo  :IAction
    {
        #region IAction 成员
        ...
        public ResultStatus DoAction(List<QueryCondition> actionParamsList)
        {
            ...
            //插入新维修工区
            res = DBCommon.Instance.InsertTable(info, "para_4042_device_info");
            if (res == 1)
            {
                Wrapper.ShowDialog("设备参数信息添加成功。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.Para_4042_Device_Info_Add, "0", "设备参数信息添加成功");
                Util.DataBase.Commit();
                return null;
            }
            else
            {
                Wrapper.ShowDialog("设备参数信息添加失败。");
                BuinessRule.GetInstace().logManager.AddLogInfo(OperationCode.Para_4042_Device_Info_Add, "1", "设备参数信息添加失败");

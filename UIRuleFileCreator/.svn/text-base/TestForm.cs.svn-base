...
public class GenerCode
{
    ...
    public static List<TableFieldProperty> GetAllColumnComments()
    {
            ...
            _GetAllColumnComments.Count <= 0)
        {
            ...
            string sqlQuery = string.Format("select atc.TABLE_NAME, atc.COLUMN_NAME,atc.DATA_TYPE,acc.comments from all_tab_columns atc inner join all_col_comments acc on atc.OWNER = acc.owner and atc.TABLE_NAME = acc.table_name and acc.column_name = atc.COLUMN_NAME where atc.OWNER = '{0}' ",DbHelper.Instance.DatabaseOwner);
